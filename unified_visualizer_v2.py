...
# Agent Administration Module (V2 Feature)
try:
    from agent_administration import (
        AgentDatabaseManager, 
        create_administration_layout,
        integrate_with_unified_visualizer,
        ADMIN_CSS
    )
    AGENT_ADMIN_AVAILABLE = True
except ImportError as e:
    AGENT_ADMIN_AVAILABLE = False
    integrate_with_unified_visualizer = None
    print(f"WARNING: Agent administration module not available: {e}")
...
app = dash.Dash(
    __name__,
    title="SecretSnipe Dashboard V2",
...
def download_windows_installer():
    ...
    installer_path = os.path.join(os.path.dirname(__file__), 
                                   'agent_project', 'windows_installer', 
                                   'Install-SecretSnipeAgent.ps1')
    
    # Also check in mounted path (for Docker)
    if not os.path.exists(installer_path):
        installer_path = '/app/agent_project/windows_installer/Install-SecretSnipeAgent.ps1'
    ...
    return Response(
        ...
        headers={
            'Content-Disposition': 'attachment; filename=Install-SecretSnipeAgent.ps1',
            'Content-Type': 'application/octet-stream; charset=utf-8'
        }
    )


@server.route('/api/download/agent/windows-readme', methods=['GET'])
def download_windows_readme():
    """View/download the Windows agent documentation"""
    from flask import Response
    
    if not session.get('authenticated'):
        return Response("Not authenticated", status=401)
    
    readme_content = '''# SecretSnipe Windows Agent Installation Guide
...
1. **Download the installer** from the Administration page
...
|-----------|-------------|---------|
| `-ManagerUrl` | URL of the SecretSnipe Manager API | Required |
| `-ApiKey` | API key for authentication | Required |
| `-InstallPath` | Installation directory | C:\\Program Files\\SecretSnipe |
| `-DataPath` | Data/config directory | C:\\ProgramData\\SecretSnipe |
| `-ServiceName` | Windows service name | SecretSnipeAgent |
...
### View Logs
```powershell
Get-Content "C:\\ProgramData\\SecretSnipe\\logs\\agent.log" -Tail 100
```

### Restart Service
```powershell
Restart-Service SecretSnipeAgent
```
...
- **DPAPI Encryption**: API keys are encrypted using Windows DPAPI
- **Service Account**: Runs under LocalSystem (can be changed)
- **Secure Permissions**: Only SYSTEM and Administrators have access to config
...
### Service Won't Start
1. Check logs: `C:\\ProgramData\\SecretSnipe\\logs\\agent.log`
...
def get_file_grouped_data(tool_filter: str = 'all', severity_filter: str = 'all', 
    ...
    try:
        ...
        # V2: Added scan_source to identify where findings came from
        query = f"""
            SELECT 
                f.file_path,
                ...
                -- V2: Scan source - 'Server' for local scans, agent hostname for remote
                COALESCE(f.scan_source, '🖥️ Server') as scan_source
            FROM findings f
            JOIN projects p ON f.project_id = p.id
            WHERE {where_clause}
            GROUP BY f.file_path, p.name, COALESCE(f.scan_source, '🖥️ Server')
            ORDER BY 
...
# ============================================================================
# V2: MULTI-PAGE LAYOUT WITH URL ROUTING
# ============================================================================
...
def create_main_dashboard_layout():
    ...
    return html.Div([
        ...
        # Main container
        html.Div([
            # Header with controls (V2 - Simplified)
            html.Div([
                html.Div([
            ...
            # V2: Simplified Scanner Status - Shows overall status only
            html.Div([
                html.H3("📊 Scan Status"),
                html.Div([
                    ...
                    html.Div([
                        ...
                        html.Span("Configure scan targets in ", style={'color': '#888'}),
                        dcc.Link("Administration", href="/administration", 
                                style={'color': '#4dabf7', 'textDecoration': 'none'}),
                    ], style={'gridColumn': 'span 2', 'textAlign': 'center', 'padding': '10px',
                             'backgroundColor': '#252525', 'borderRadius': '6px', 'fontSize': '13px'})
            ...
            html.Div([
                ...
                dcc.Loading(
                    ...
                    style_cell_conditional=[
                        {'if': {'column_id': 'parent_folder'}, 'width': '150px', 'maxWidth': '170px',
                         'overflow': 'hidden', 'textOverflow': 'ellipsis', 'color': '#9ca3af', 'fontSize': '11px'},
                        {'if': {'column_id': 'file_path'}, 'width': '30%', 'maxWidth': '300px', 
                         'overflow': 'hidden', 'textOverflow': 'ellipsis'},
                        {'if': {'column_id': 'finding_count'}, 'width': '70px', 'textAlign': 'center', 
                         'fontWeight': 'bold'},
                        {'if': {'column_id': 'max_severity'}, 'width': '90px', 'textAlign': 'center'},
                        {'if': {'column_id': 'scan_source'}, 'width': '100px', 'textAlign': 'center'},
                        {'if': {'column_id': 'tools_display'}, 'width': '100px', 'textAlign': 'center'},
                        {'if': {'column_id': 'types_display'}, 'width': '130px'},
                        {'if': {'column_id': 'latest_finding'}, 'width': '90px', 'textAlign': 'center'},
                    ],
                    style_data_conditional=[
...
# ============================================================================
# V2: ROOT LAYOUT WITH URL ROUTING
# ============================================================================

def create_layout():
    """V2: Create the root layout with URL-based page routing"""
    return html.Div([
        ...
        html.Div([
            html.Div([
                html.Span("🔒 SecretSnipe", style={'fontSize': '20px', 'fontWeight': 'bold', 'color': '#e0e0e0'}),
                html.Span(" V2", style={'fontSize': '12px', 'color': '#667eea', 'marginLeft': '5px'})
            ]),
            html.Div([
                dcc.Link("📊 Dashboard", href="/", className="nav-link", 
                        style={'marginRight': '20px', 'color': '#e0e0e0', 'textDecoration': 'none'}),
                dcc.Link("🛡️ Administration", href="/administration", className="nav-link",
...
def display_page(pathname):
    ...
    if pathname == '/administration':
        if AGENT_ADMIN_AVAILABLE:
            return create_administration_layout()
        else:
            return html.Div([
                html.H2("⚠️ Agent Administration Not Available"),
                html.P("The agent administration module could not be loaded."),
                html.P("Make sure agent_administration.py is in the same directory."),
                dcc.Link("← Back to Dashboard", href="/")
            ], style={'padding': '50px', 'textAlign': 'center', 'color': '#e0e0e0'})
    else:
...
# V2: Integrate Agent Administration callbacks
if AGENT_ADMIN_AVAILABLE and integrate_with_unified_visualizer:
    try:
        agent_db_manager = integrate_with_unified_visualizer(app, db_manager)
        logger.info("✅ Agent Administration callbacks registered")
    except Exception as e:
        logger.error(f"❌ Failed to integrate Agent Administration: {e}")
        AGENT_ADMIN_AVAILABLE = False
...
<html>
    <head>
        <title>SecretSnipe Dashboard V2</title>
...
def main():
    ...
    logger.info("🔒 Initializing SecretSnipe Dashboard V2 with Agent Administration...")
    ...
    audit_log('dashboard_startup', 'system', {
        ...
        'features': ['agent_administration'] if AGENT_ADMIN_AVAILABLE else []
    })
    ...
    if AGENT_ADMIN_AVAILABLE:
        logger.info("✅ Agent Administration module loaded")
    else:
        logger.warning("⚠️ Agent Administration module not available")

    # Security configuration validation
    dashboard_host = config.dashboard.host
    # V2: Use port 8052 for testing, or override from config
    dashboard_port = int(os.environ.get('DASHBOARD_PORT', 8052))  # V2 default: 8052
    ...
    logger.info("🚀 Starting SecretSnipe Dashboard V2...")
    logger.info(f"📊 Dashboard: http://{dashboard_host}:{dashboard_port}")
    logger.info(f"🛡️ Administration: http://{dashboard_host}:{dashboard_port}/administration")
